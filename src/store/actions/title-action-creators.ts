...
export const fetchTitles = () => {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const titles = await (isAnime
        ? fetchAnimeData.fetchAnimes()
        : fetchMangaData.fetchManga());
...
export const fetchTitleGenres = () => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const titleGenres = await (isAnime
        ? fetchAnimeData.fetchAnimeGenres()
        : fetchMangaData.fetchMangaGenres());
...
export const fetchRandomTitle = () => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const randomTitles = await (isAnime
        ? fetchAnimeData.fetchRandomAnime()
        : fetchMangaData.fetchRandomManga());
...
export const fetchPaginatedTitles = (page: number) => {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const titles = await (isAnime
        ? fetchAnimeData.fetchPaginatedAnimes(page)
        : fetchMangaData.fetchPaginatedManga(page));
...
export const fetchPaginatedTitlesByGenre = (
  ...
  return async (dispatch: Dispatch) => {
    ...
    try {
      const titles = await (isAnime
        ? fetchAnimeData.fetchAnimesByGenres(genre, page)
        : fetchMangaData.fetchMangasByGenres(genre, page));
      dispatch(setLastTitlePage(titles.data.pagination.last_visible_page));
...
export const fetchTitleSearch = (inputValue: string | null, page: number) => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const searchResult = await (isAnime
        ? fetchAnimeData.fetchAnimeSearch(inputValue, page)
        : fetchMangaData.fetchMangaSearch(inputValue, page));
      dispatch(
        setLastTitlePage(searchResult.data.pagination.last_visible_page),
      );
...
export const fetchCurrentTitle = (id: string | undefined) => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const currentTitle = await (isAnime
        ? fetchAnimeData.fetchCurrentAnimeTitle(id)
        : fetchMangaData.fetchCurrentMangaTitle(id));
...
export const fetchPopularTitle = (page: number) => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const popularTitle = await (isAnime
        ? fetchAnimeData.fetchPopularAnime(page)
        : fetchMangaData.fetchPopularManga(page));
      dispatch(setPopularTitle(popularTitle.data.data));
      dispatch(
        setLastTitlePage(popularTitle.data.pagination.last_visible_page),
      );
      dispatch(isTitleFlag(topic));
    } catch (error) {
      dispatch(setTitleError(true));
...
export const fetchTitleReviews = (id: number | undefined) => {
  return async (dispatch: Dispatch) => {
    ...
    const isAnime = topic === 'anime';
    try {
      const titleReview = await (isAnime
        ? fetchAnimeData.fetchAnimeReviews(id)
        : fetchMangaData.fetchMangaReviews(id));
