    ...
    <div className="current-title-wrapper">
      <div className="title">
        <div className="title-item">
          <img
            className="title-poster-current"
            src={title.images?.webp.large_image_url}
            alt={`${title.title}-poster`}
          />
          <div className="title-info">
            <p className="title-title_name">
            ...
            <p style={{ color: getScoreColor(score) }}>
              {titleScore === null ? null : `Score: ${score}`}
            </p>
            <p>
              {title.scored_by === null
                ? null
                : `Scored by: ${title.scored_by} users`}{' '}
            </p>
            <div className="title-genres">
              {title.genres?.map((genre: IGenre) => (
                <p className="title-name" key={genre.mal_id}>
                  {genre.name}
                </p>
              ))}
            </div>
            ...
            <Link className="title-link" to={`/reviews/${id}`}>
              See reviews ({titleReviews ? titleReviews.length : 0})
            </Link>
          </div>
        </div>
      </div>
