...
export const Reviews = () => {
  ...
  const { titleReviews, currentTitle } = useTypedSelector(
    (state) => state.title,
  );
  ...
  return titleReviews.length === 0 ? (
    ...
    <div className="review-wrapper">
      <div className="review-list">
        {currentTitle ? (
          <h1 className="review-list-title" id="top">
            Review's on {currentTitle.title}
          </h1>
        ) : (
          <h1>Loading..</h1>
        )}
        {titleReviews &&
          titleReviews.map((review: IReview) => (
            <div className="review" key={review.mal_id}>
              <div className="user">
                ...
                <p className="user-username">
                  Review by <i>{review?.user?.username}</i>
                </p>
              </div>
