...
export const fetchAnimeData = {
  ...
  async fetchRandomAnime() {
    ...
    const randomAnimes = [
      randomTitle1.data.data,
      randomTitle2.data.data,
      randomTitle3.data.data,
    ];
    return randomAnimes;
  },
  async fetchAnimeSearch(inputValue: string | null, page: number) {
    const animes = await axiosGet(
      `${URL_ANIME_SEARCH}?q=${inputValue}&page=${page}&limit=5&order_by=score&sort=desc`,
    );
    return animes;
  },
  ...
  async fetchAnimesByGenres(genre: string | undefined, page: number) {
    const animesByGenres = await axiosGet(
      `${URL_ANIME_SEARCH}?genres=${genre}&page=${page}&limit=5&sfw=false&order_by=score&sort=desc`,
    );
...
export const fetchMangaData = {
  ...
  async fetchMangaSearch(inputValue: string | null, page: number) {
    const mangas = await axiosGet(
      `${URL_MANGA_SEARCH}?q=${inputValue}&page=${page}&limit=5&order_by=score&sort=desc`,
    );
    return mangas;
  },
  ...
  async fetchMangasByGenres(genre: string | undefined, page: number) {
    const mangaByGenres = await axiosGet(
      `${URL_MANGA_SEARCH}?genres=${genre}&page=${page}&limit=5&sfw=false&order_by=score&sort=desc`,
    );
